...
export const copy = {
  title: 'Be the CISO',
  intro:
    "You're the CISO at Acme, a fictional company, and the quarterly access review just " +
  ...
  steps: {
    ...
    decide: {
      ...
      body:
        'Each card shows the facts, why it could be an issue, and a proposal. Confirm ' +
        'accepts every proposal at once. Keeping something proposed for revocation, or ' +
        'overriding a proposal, asks for a reason.',
      aws:
        "Each click goes to a Lambda function URL that checks Slack's signature first. " +
    ...
    signoff: {
      title: 'Sign off',
      body:
        "Every decision in one list, bound to the manifest's SHA-256. Check it, then " + 'approve.',
      aws:
      ...
      title: 'The tickets',
      body:
        'Signed off. One Jira ticket for each revoke and each finding to fix, with the ' +
        'checklist in your DM and a summary in the channel.',
      aws:
        'The workflow opens the tickets from the signed decisions, never from a new count. ' +
    ...
    evidence: {
      ...
      body:
        "An auditor downloads the review from S3 and runs the tool's check. Your " +
        'browser hashes the files right here. Change one byte and see what it says.',
      aws:
        'Everything the review wrote sits in S3 under Object Lock. The check hashes each ' +
        'file against the manifest, and the sign-off against both hashes.',
      scope:
        "This page holds three of the files and hashes those. The run's other six files " +
