...
# the qa.<domain> zone, the production certificate, the CloudFront policies, alerts and the
# budget. infra/envs/{prod,qa} (CI) build the sites on top of it.
...
locals {
  ...
  bucket_suffix = "${local.account_id}-${var.region}-an"
